...
#   python scripts/plotting_scripts/performance_map.py --params_csv results/final_results/Modified_LPG68/Fitting/fitted_params_lpg68_modified_ga_2026-03-22_185546.csv --oil LPG68
#
...
def compute_efficiency_point(
    ...
    """
    Returns (eta_is, lambda_h) or (nan, nan) on failure.
    """
    inputs = SimpleInputs(
...
def compute_grid(
    ...
    """
    Compute eta_is and lambda_h over a 2D T_evap × T_cond grid.
    Returns (eta_grid, lambda_grid) as 2D arrays of shape (n_T_cond, n_T_evap).
    """
    ...
    for j, T_cond_C in enumerate(T_cond_grid):
        for i, T_evap_C in enumerate(T_evap_grid):
            ...
            eta_is, lambda_h = compute_efficiency_point(
...
def main():
    ap = argparse.ArgumentParser(
        description="Performance map heatmaps: eta_is and lambda_h over T_evap × T_cond grid."
    )
    ...
    # Model / fluid
    ap.add_argument("--model", default="auto", help="original | modified | auto")
    ...
    # Oil name only needed for modified model
    ...
    # Compute grids
    # -------------------------
    eta_grid, lambda_grid = compute_grid(
    ...
    eta_path = out_dir / f"perfmap_eta_is_{oil_tag}_N{int(args.N_rpm)}_SH{int(args.SH_K)}_{stamp}.{args.out_format}"
    plot_heatmap(
        grid=eta_grid,
        ...
        title=f"Isentroper Wirkungsgrad\n{subtitle}",
        out_path=eta_path,
        cmap=args.cmap_eta,
    ...
    # -------------------------
    # Plot lambda_h heatmap
    # -------------------------
    lambda_path = out_dir / f"perfmap_lambda_h_{oil_tag}_N{int(args.N_rpm)}_SH{int(args.SH_K)}_{stamp}.{args.out_format}"
    plot_heatmap(
        grid=lambda_grid,
        T_evap_grid=T_evap_grid,
        T_cond_grid=T_cond_grid,
        metric_label="lambda_h",
