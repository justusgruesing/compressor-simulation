# scripts/fit_molinaroli_ga.py
#
# Simple Genetic Algorithm fitter (James et al. 2016 style / Fig. A2),
# adapted to Molinaroli compressor models and your dataset layout.
#
# Supports:
#   - original model
#   - modified model with alpha_fric_tot
#
# Objective includes:
#   - mass flow
#   - electric power
#   - discharge temperature
#   (temp error normalized by Tdis_norm_K, e.g. 50 K as in Eq. (40))
#
# Outputs:
#   - fitted_params_<oil>_<model>_ga_<timestamp>.csv
#   - fit_predictions_<oil>_<model>_ga_<timestamp>.csv
#   - optional shared split definition CSV
#
# Notes on training split:
#   --train_selection_mode rows
#       selects individual rows randomly (old behavior)
#
#   --train_selection_mode operating_points
#       selects unique operating-point keys randomly; all rows with the same
#       operating-point key are used as train rows. This is useful if you want
#       the same operating points across LPG68 / LPG100 / all-oil fits.
#
#   --shared_split_csv <path>
#       If the file exists, the split is loaded from it.
#       If the file does not exist, the split is created and saved there.
#
# Example:
#   python scripts/fit_molinaroli_ga.py --csv data/Datensatz_Fitting_2.csv \
#       --oil LPG68 --model modified --refrigerant PROPANE \
#       --train_selection_mode operating_points \
#       --shared_split_csv results/shared_split_ops.csv \
#       --n_train 20 --generations 50 --population 20
#
from __future__ import annotations

import argparse
import multiprocessing as mp
import os
from concurrent.futures import ProcessPoolExecutor
from dataclasses import dataclass
from datetime import datetime
from pathlib import Path

import tempfile
import shutil
import atexit
import uuid
import warnings
import re

import numpy as np
import pandas as pd

from vclibpy.components.compressors import (
    Molinaroli_2017_Compressor,
    Molinaroli_2017_Compressor_Modified,
)
from vclibpy.datamodels import FlowsheetState
from vclibpy.media import RefProp


# =============================================================================
# Warning handling
# =============================================================================
def _short_refprop_warning(message: str) -> str:
    s = str(message)

    m = re.search(
        r"Temperature above upper limit: T\s*=\s*([0-9.]+)\s*K,\s*Tmax\s*=\s*([0-9.]+)\s*K",
        s,
    )
    if m:
        return f"RefProp: T>Tmax (T={m.group(1)}K, Tmax={m.group(2)}K)"

    first = s.splitlines()[0].strip()
    return (first[:140] + "…") if len(first) > 140 else first


def _warn_handler(message, category, filename, lineno, file=None, line=None):
    msg = str(message)

    if "ref_prop.py" in filename or "REFPROP" in msg or "PSFLSH" in msg or "PHFLSH" in msg:
        print(_short_refprop_warning(msg))
        return

    print(warnings.formatwarning(message, category, filename, lineno, line))


warnings.showwarning = _warn_handler


# =============================================================================
# CSV column defaults
# =============================================================================
OIL_COL_DEFAULT = "Ölbezeichnung"
P_SUC_COL_DEFAULT = "P1_mean"            # bar
T_SUC_COL_DEFAULT = "T1_mean"            # °C
P_OUT_COL_DEFAULT = "P2_mean"            # bar
T_AMB_COL_DEFAULT = "Tamb_mean"          # °C
SPEED_COL_DEFAULT = "N"                  # rpm
M_FLOW_MEAS_COL_DEFAULT = "suction_mf_mean"  # g/s
P_EL_MEAS_COL_DEFAULT = "Pel_mean"       # W
T_DIS_MEAS_COL_DEFAULT = "T2_mean"       # °C


# =============================================================================
# Fixed references
# =============================================================================
F_REF = 50.0
T_REF = 273.15
Q_REF = 1.0

FAIL_E = 1e3  # failed simulation penalty


# =============================================================================
# Model-specific parameter configs
# =============================================================================
PARAM_NAMES_ORIGINAL = [
    "Ua_suc_ref",
    "Ua_dis_ref",
    "Ua_amb",
    "A_tot",
    "A_dis",
    "V_IC",
    "alpha_loss",
    "W_dot_loss_ref",
]

PARAM_NAMES_MODIFIED = [
    "Ua_suc_ref",
    "Ua_dis_ref",
    "Ua_amb",
    "A_tot",
    "A_dis",
    "V_IC",
    "alpha_loss",
    "W_dot_loss_ref",
    "alpha_fric_tot",
]

DEFAULT_PARAMS_ORIGINAL = {
    "Ua_suc_ref": 16.05,
    "Ua_dis_ref": 13.96,
    "Ua_amb": 0.36,
    "A_tot": 9.47e-9,
    "A_dis": 86.1e-6,
    "V_IC": 30.7e-6,
    "alpha_loss": 0.16,
    "W_dot_loss_ref": 83.0,
    "m_dot_ref": None,
    "f_ref": F_REF,
}

DEFAULT_PARAMS_MODIFIED = {
    "Ua_suc_ref": 16.05,
    "Ua_dis_ref": 13.96,
    "Ua_amb": 0.36,
    "A_tot": 9.47e-9,
    "A_dis": 86.1e-6,
    "V_IC": 30.7e-6,
    "alpha_loss": 0.16,
    "W_dot_loss_ref": 10.0,
    "alpha_fric_tot": 500.0,
    "m_dot_ref": None,
    "f_ref": F_REF,
}

# log-uniform only for A_tot and A_dis
LOG_UNIFORM_IDX = {3, 4}


def get_param_config(model: str):
    m = str(model).lower().strip()
    if m in ("orig", "original"):
        return PARAM_NAMES_ORIGINAL, DEFAULT_PARAMS_ORIGINAL
    if m in ("mod", "modified"):
        return PARAM_NAMES_MODIFIED, DEFAULT_PARAMS_MODIFIED
    raise ValueError("Unknown model. Use original | modified")


def get_bounds(model: str, V_h_m3: float, vic_lo_scale: float, vic_hi_scale: float) -> np.ndarray:
    vic_lo = vic_lo_scale * V_h_m3
    vic_hi = vic_hi_scale * V_h_m3

    m = str(model).lower().strip()

    if m in ("orig", "original"):
        return np.array([
            [2.0, 55.0],        # Ua_suc_ref
            [2.0, 50.0],        # Ua_dis_ref
            [0.1, 3.0],         # Ua_amb
            [7e-9, 5e-7],       # A_tot (log-uniform)
            [4e-6, 5e-4],       # A_dis (log-uniform)
            [vic_lo, vic_hi],   # V_IC
            [0.10, 0.30],       # alpha_loss
            [40.0, 175.0],      # W_dot_loss_ref
        ], dtype=float)

    if m in ("mod", "modified"):
        return np.array([
            [2.0, 55.0],        # Ua_suc_ref
            [2.0, 50.0],        # Ua_dis_ref
            [0.1, 3.0],         # Ua_amb
            [7e-9, 5e-7],       # A_tot (log-uniform)
            [4e-6, 5e-4],       # A_dis (log-uniform)
            [vic_lo, vic_hi],   # V_IC
            [0.10, 0.30],       # alpha_loss
            [0.0, 120.0],       # W_dot_loss_ref
            [10.0, 5000.0],     # alpha_fric_tot
        ], dtype=float)

    raise ValueError("Unknown model. Use original | modified")


# =============================================================================
# Unit conversions
# =============================================================================
def bar_to_pa(p): return float(p) * 1e5
def c_to_k(t): return float(t) + 273.15
def rpm_to_hz(n): return float(n) / 60.0
def gs_to_kgps(m): return float(m) / 1000.0


# =============================================================================
# Name helpers
# =============================================================================
def norm_oil_name(s: str) -> str:
    s0 = str(s).strip().lower().replace(" ", "")
    if s0 == "lpg68":
        return "LPG68"
    if s0 == "lpg100":
        return "LPG100"
    return str(s).strip()


def map_refrigerant_for_modified_model(name: str) -> str:
    s = str(name).strip().upper()
    if s in ("PROPANE", "R290"):
        return "propane"
    return str(name).strip().lower()


def map_oil_for_modified_model(name: str) -> str:
    s = norm_oil_name(name)
    if s == "LPG68":
        return "LPG 68"
    if s == "LPG100":
        return "LPG 100"
    raise ValueError(f"Unsupported oil for modified model: {name}")


# =============================================================================
# Operating-point key
# =============================================================================
def make_operating_point_key(f_oper_hz, p_suc_bar, T_suc_C, p_out_bar, T_amb_C) -> str:
    """
    String key for operating-point-based train split.

    Important:
    This key is based on measured operating variables. It works well if the same
    operating points are represented by the same measured values across fits.
    """
    return (
        f"f={float(f_oper_hz):.8f}|"
        f"ps={float(p_suc_bar):.8f}|"
        f"Ts={float(T_suc_C):.8f}|"
        f"po={float(p_out_bar):.8f}|"
        f"Ta={float(T_amb_C):.8f}"
    )


# =============================================================================
# Model helpers
# =============================================================================
def make_compressor(
    model: str,
    N_max_hz: float,
    V_h_m3: float,
    params: dict,
    refrigerant_name: str | None = None,
    oil_name: str | None = None,
):
    m = str(model).lower().strip()

    if m in ("orig", "original"):
        return Molinaroli_2017_Compressor(
            N_max=N_max_hz,
            V_h=V_h_m3,
            parameters=params,
        )

    if m in ("mod", "modified"):
        if refrigerant_name is None:
            raise ValueError("Modified model requires refrigerant_name.")
        if oil_name is None:
            raise ValueError("Modified model requires oil_name.")

        return Molinaroli_2017_Compressor_Modified(
            N_max=N_max_hz,
            V_h=V_h_m3,
            fluid_name=map_refrigerant_for_modified_model(refrigerant_name),
            lub_name=map_oil_for_modified_model(oil_name),
            parameters=params,
        )

    raise ValueError("Unknown model. Use original | modified")


def compute_m_dot_ref(med, V_h_m3: float) -> float:
    st = med.calc_state("TQ", T_REF, Q_REF)
    return float(st.d) * float(V_h_m3) * F_REF


def x_to_params(x: np.ndarray, param_names: list[str], default_params: dict) -> dict:
    p = dict(default_params)
    for name, val in zip(param_names, x):
        p[name] = float(val)
    return p


def read_dataset_csv(path, sep, header, decimal):
    return pd.read_csv(path, sep=sep, header=header, decimal=decimal)


# =============================================================================
# Inputs dataclasses
# =============================================================================
@dataclass
class Control:
    n: float


@dataclass
class SimpleInputs:
    control: Control
    T_amb: float
    lsq_max_nfev: int = 20000
    lsq_ftol: float = 1e-8
    lsq_xtol: float = 1e-8


def _clamp01(x):
    return max(1e-9, min(1.0, float(x)))


# =============================================================================
# Single operating point
# =============================================================================
def simulate_point(
    comp,
    med,
    inputs: SimpleInputs,
    fs_state: FlowsheetState,
    p_suc_pa,
    T_suc_K,
    p_out_pa,
    n_rel,
    T_amb_K,
):
    inputs.control.n = _clamp01(n_rel)
    inputs.T_amb = float(T_amb_K)

    comp.state_inlet = med.calc_state("PT", float(p_suc_pa), float(T_suc_K))
    comp.calc_state_outlet(p_outlet=float(p_out_pa), inputs=inputs, fs_state=fs_state)

    m_flow = float(comp.m_flow)
    P_el = float(comp.P_el)
    T_dis_K = float(comp.state_outlet.T)

    if not np.isfinite(m_flow) or m_flow <= 0:
        raise ValueError("Invalid m_flow")
    if not np.isfinite(P_el) or P_el <= 0:
        raise ValueError("Invalid P_el")
    if not np.isfinite(T_dis_K) or T_dis_K <= 0:
        raise ValueError("Invalid T_dis")

    return m_flow, P_el, T_dis_K


# =============================================================================
# Objective function
# =============================================================================
def objective_error(
    x,
    rows,
    med,
    model,
    N_max_hz,
    V_h_m3,
    m_dot_ref,
    use_Tdis,
    Tdis_norm_K,
    lsq_max_nfev,
    param_names,
    default_params,
    refrigerant_name,
):
    """
    error = Σ_i [ (Δm/m_meas)^2 + (ΔW/W_meas)^2 + (ΔT/T_norm)^2 ]

    Original model:
        one compressor per individual

    Modified model:
        one compressor per oil per individual
        (same shared parameter vector, oil-specific lubricant model)
    """
    params = x_to_params(np.asarray(x, dtype=float), param_names, default_params)
    params["f_ref"] = F_REF
    params["m_dot_ref"] = float(m_dot_ref)

    model_name = str(model).lower().strip()
    comp_cache: dict[str, object] = {}

    def _get_comp_for_row(row):
        if model_name in ("orig", "original"):
            key = "__single__"
            oil_name = None
        else:
            key = str(row["oil_norm"])
            oil_name = str(row["oil_norm"])

        if key not in comp_cache:
            comp = make_compressor(
                model=model,
                N_max_hz=N_max_hz,
                V_h_m3=V_h_m3,
                params=params,
                refrigerant_name=refrigerant_name,
                oil_name=oil_name,
            )
            comp.med_prop = med
            comp_cache[key] = comp
        return comp_cache[key]

    inputs = SimpleInputs(
        control=Control(n=1e-6),
        T_amb=298.15,
        lsq_max_nfev=int(lsq_max_nfev),
        lsq_ftol=1e-8,
        lsq_xtol=1e-8,
    )
    fs_state = FlowsheetState()

    err = 0.0
    for r in rows:
        try:
            comp = _get_comp_for_row(r)
            m_c, P_c, T_c = simulate_point(
                comp,
                med,
                inputs,
                fs_state,
                r["p_suc_pa"],
                r["T_suc_K"],
                r["p_out_pa"],
                r["n_rel"],
                r["T_amb_K"],
            )
            err += ((m_c - r["m_meas"]) / r["m_meas"]) ** 2
            err += ((P_c - r["P_meas"]) / r["P_meas"]) ** 2
            if use_Tdis and r.get("T_dis_meas_K") is not None:
                err += ((T_c - r["T_dis_meas_K"]) / float(Tdis_norm_K)) ** 2
        except Exception:
            err += 2.0 * FAIL_E ** 2
            if use_Tdis:
                err += FAIL_E ** 2

    return float(err)


# =============================================================================
# Parallelization: worker globals
# =============================================================================
_WORK: dict = {}


def _init_worker(
    refrigerant,
    model,
    N_max_hz,
    V_h_m3,
    m_dot_ref,
    rows_train,
    use_Tdis,
    Tdis_norm_K,
    lsq_max_nfev,
    param_names,
    default_params,
):
    global _WORK

    pid = os.getpid()
    unique = uuid.uuid4().hex[:8]
    worker_dir = Path(tempfile.gettempdir()) / f"refprop_worker_{pid}_{unique}"
    worker_dir.mkdir(parents=True, exist_ok=True)

    os.chdir(worker_dir)

    def _cleanup():
        try:
            shutil.rmtree(worker_dir, ignore_errors=True)
        except Exception:
            pass

    atexit.register(_cleanup)

    try:
        med = RefProp(fluid_name=refrigerant)
    except TypeError:
        med = RefProp(refrigerant)

    _WORK = {
        "med": med,
        "model": str(model),
        "N_max_hz": float(N_max_hz),
        "V_h_m3": float(V_h_m3),
        "m_dot_ref": float(m_dot_ref),
        "rows_train": rows_train,
        "use_Tdis": bool(use_Tdis),
        "Tdis_norm_K": float(Tdis_norm_K),
        "worker_dir": str(worker_dir),
        "lsq_max_nfev": int(lsq_max_nfev),
        "param_names": list(param_names),
        "default_params": dict(default_params),
        "refrigerant_name": str(refrigerant),
    }


def _objective_error_worker(x_in) -> float:
    x = np.asarray(x_in, dtype=float).reshape(-1)
    return objective_error(
        x=x,
        rows=_WORK["rows_train"],
        med=_WORK["med"],
        model=_WORK["model"],
        N_max_hz=_WORK["N_max_hz"],
        V_h_m3=_WORK["V_h_m3"],
        m_dot_ref=_WORK["m_dot_ref"],
        use_Tdis=_WORK["use_Tdis"],
        Tdis_norm_K=_WORK["Tdis_norm_K"],
        lsq_max_nfev=_WORK["lsq_max_nfev"],
        param_names=_WORK["param_names"],
        default_params=_WORK["default_params"],
        refrigerant_name=_WORK["refrigerant_name"],
    )


# =============================================================================
# GA helpers
# =============================================================================
def _sample_param(lo, hi, rng, log_uniform):
    lo, hi = float(lo), float(hi)
    if log_uniform and lo > 0 and hi > 0:
        return float(10.0 ** rng.uniform(np.log10(lo), np.log10(hi)))
    return float(rng.uniform(lo, hi))


def random_individual(bounds, rng):
    return np.array([
        _sample_param(bounds[j, 0], bounds[j, 1], rng, j in LOG_UNIFORM_IDX)
        for j in range(bounds.shape[0])
    ], dtype=float)


def uniform_crossover(p1, p2, rng):
    mask = rng.random(p1.size) < 0.5
    return np.where(mask, p1, p2).astype(float)


def mutate(child, bounds, rng, p_mut):
    for j in range(child.size):
        if rng.random() < p_mut:
            child[j] = _sample_param(bounds[j, 0], bounds[j, 1], rng, j in LOG_UNIFORM_IDX)
    return child


# =============================================================================
# Data loading
# =============================================================================
def build_rows(df, args, N_max_hz):
    has_Tdis = args.col_T_dis in df.columns
    rows = []

    if args.model == "modified" and args.oil_col not in df.columns:
        raise ValueError("Modified model requires an oil column in the dataset.")

    for i, r in df.iterrows():
        m_meas = gs_to_kgps(r[args.col_m_meas])
        P_meas = float(r[args.col_P_meas])

        if m_meas <= 0 or P_meas <= 0 or not np.isfinite(m_meas) or not np.isfinite(P_meas):
            continue

        T_dis_meas_K = None
        if has_Tdis and pd.notna(r[args.col_T_dis]):
            T_dis_meas_K = c_to_k(r[args.col_T_dis])

        oil_value = str(r[args.oil_col]) if args.oil_col in df.columns else str(args.oil)
        oil_norm = norm_oil_name(oil_value)

        f_oper_hz = rpm_to_hz(r[args.col_speed])
        p_suc_bar = float(r[args.col_p_suc])
        T_suc_C = float(r[args.col_T_suc])
        p_out_bar = float(r[args.col_p_out])
        T_amb_C = float(r[args.col_T_amb])

        op_key = make_operating_point_key(
            f_oper_hz=f_oper_hz,
            p_suc_bar=p_suc_bar,
            T_suc_C=T_suc_C,
            p_out_bar=p_out_bar,
            T_amb_C=T_amb_C,
        )

        rows.append({
            "idx_all": len(rows),
            "source_row_index": int(i),
            "oil": oil_value,
            "oil_norm": oil_norm,
            "op_key": op_key,
            "p_suc_pa": bar_to_pa(p_suc_bar),
            "T_suc_K": c_to_k(T_suc_C),
            "p_out_pa": bar_to_pa(p_out_bar),
            "T_amb_K": c_to_k(T_amb_C),
            "f_oper_hz": f_oper_hz,
            "n_rel": _clamp01(f_oper_hz / N_max_hz),
            "m_meas": m_meas,
            "P_meas": P_meas,
            "T_dis_meas_K": float(T_dis_meas_K) if T_dis_meas_K is not None else None,
        })

    if not rows:
        raise ValueError("No valid rows after filtering.")

    return rows, has_Tdis


# =============================================================================
# Shared split helpers
# =============================================================================
def load_shared_split(path: Path, selection_mode: str) -> set[str]:
    df = pd.read_csv(path, dtype={"selection_id": str})

    required = {"selection_id", "is_train"}
    if not required.issubset(df.columns):
        raise ValueError(f"Shared split CSV missing required columns: {sorted(required - set(df.columns))}")

    if "selection_mode" in df.columns:
        modes = set(df["selection_mode"].astype(str).str.strip().str.lower().unique().tolist())
        if len(modes) == 1 and selection_mode.lower() not in modes:
            raise ValueError(
                f"Shared split CSV was created for selection_mode={list(modes)[0]!r}, "
                f"but current run uses {selection_mode!r}."
            )

    train_ids = set(
        df.loc[df["is_train"].astype(str).str.strip().str.lower().isin({"1", "true", "t", "yes", "y"}), "selection_id"]
        .astype(str)
        .tolist()
    )
    return train_ids


def save_shared_split(path: Path, selection_mode: str, all_ids: list[str], train_ids: set[str]) -> None:
    out = pd.DataFrame({"selection_id": [str(x) for x in all_ids]})
    out["is_train"] = out["selection_id"].isin(train_ids)
    out["selection_mode"] = str(selection_mode)
    path.parent.mkdir(parents=True, exist_ok=True)
    out.to_csv(path, index=False)


def select_training_rows(rows_all, args, rng):
    mode = str(args.train_selection_mode).lower().strip()
    shared_path = Path(args.shared_split_csv) if args.shared_split_csv else None

    if mode == "rows":
        all_ids = [str(i) for i in range(len(rows_all))]

        if shared_path is not None and shared_path.exists():
            train_ids = load_shared_split(shared_path, selection_mode=mode)
        else:
            if args.n_train and 0 < args.n_train < len(rows_all):
                idx = np.sort(rng.choice(len(rows_all), size=args.n_train, replace=False))
                train_ids = {str(i) for i in idx}
            else:
                train_ids = set(all_ids)

            if shared_path is not None:
                save_shared_split(shared_path, selection_mode=mode, all_ids=all_ids, train_ids=train_ids)

        is_train_mask = np.array([str(i) in train_ids for i in range(len(rows_all))], dtype=bool)
        train_units_total = len(all_ids)
        train_units_selected = len(train_ids)

    elif mode == "operating_points":
        all_ids = sorted({str(r["op_key"]) for r in rows_all})

        if shared_path is not None and shared_path.exists():
            train_ids = load_shared_split(shared_path, selection_mode=mode)
        else:
            if args.n_train and 0 < args.n_train < len(all_ids):
                sel = rng.choice(len(all_ids), size=args.n_train, replace=False)
                train_ids = {all_ids[i] for i in np.sort(sel)}
            else:
                train_ids = set(all_ids)

            if shared_path is not None:
                save_shared_split(shared_path, selection_mode=mode, all_ids=all_ids, train_ids=train_ids)

        is_train_mask = np.array([str(r["op_key"]) in train_ids for r in rows_all], dtype=bool)
        train_units_total = len(all_ids)
        train_units_selected = len(train_ids)

    else:
        raise ValueError("Unknown --train_selection_mode. Use rows | operating_points")

    rows_train = [r for r, keep in zip(rows_all, is_train_mask) if keep]
    if not rows_train:
        raise ValueError("Training selection produced zero training rows.")

    return rows_train, is_train_mask, train_units_selected, train_units_total


# =============================================================================
# main
# =============================================================================
def main():
    ap = argparse.ArgumentParser(
        description="GA fit (James et al. 2016) for Molinaroli compressor models."
    )

    ap.add_argument("--csv", required=True, type=Path)
    ap.add_argument("--oil", default="all")
    ap.add_argument("--oil_col", default=OIL_COL_DEFAULT)
    ap.add_argument("--model", default="original", choices=["original", "modified"])
    ap.add_argument("--refrigerant", default="PROPANE")

    ap.add_argument("--N_max_rpm", type=float, default=7200.0)
    ap.add_argument(
        "--V_h_cm3",
        type=float,
        default=30.7,
        help="Geometrisches Hubvolumen [cm³] für m_dot_ref (kein Fit-Parameter)",
    )

    ap.add_argument("--sep", default=";")
    ap.add_argument("--decimal", default=",")
    ap.add_argument("--header", type=int, default=1)

    ap.add_argument("--col_p_suc", default=P_SUC_COL_DEFAULT)
    ap.add_argument("--col_T_suc", default=T_SUC_COL_DEFAULT)
    ap.add_argument("--col_p_out", default=P_OUT_COL_DEFAULT)
    ap.add_argument("--col_T_amb", default=T_AMB_COL_DEFAULT)
    ap.add_argument("--col_speed", default=SPEED_COL_DEFAULT)
    ap.add_argument("--col_m_meas", default=M_FLOW_MEAS_COL_DEFAULT)
    ap.add_argument("--col_P_meas", default=P_EL_MEAS_COL_DEFAULT)
    ap.add_argument("--col_T_dis", default=T_DIS_MEAS_COL_DEFAULT)

    # GA settings
    ap.add_argument("--population", type=int, default=20)
    ap.add_argument("--elite_frac", type=float, default=0.20)
    ap.add_argument("--random_keep_prob", type=float, default=0.10)
    ap.add_argument("--mutation_prob_param", type=float, default=0.20)
    ap.add_argument("--generations", type=int, default=1000)

    # Split settings
    ap.add_argument(
        "--n_train",
        type=int,
        default=0,
        help=(
            "Anzahl Trainings-Einheiten. "
            "Bei train_selection_mode=rows => Anzahl Trainingszeilen. "
            "Bei train_selection_mode=operating_points => Anzahl Trainings-Betriebspunkte. "
            "0 = alle."
        ),
    )
    ap.add_argument("--seed", type=int, default=1)
    ap.add_argument(
        "--train_selection_mode",
        choices=["rows", "operating_points"],
        default="rows",
        help="Train split based on rows or unique operating points.",
    )
    ap.add_argument(
        "--shared_split_csv",
        type=Path,
        default=None,
        help="Optional path to save/load a reusable train split definition.",
    )

    ap.add_argument("--Tdis_norm_K", type=float, default=50.0)

    # Bounds scaling for V_IC
    ap.add_argument("--vic_lo_scale", type=float, default=0.5)
    ap.add_argument("--vic_hi_scale", type=float, default=2.0)

    ap.add_argument("--out_dir", default="results/ga_fit")

    # Parallelization
    ap.add_argument(
        "--n_jobs",
        type=int,
        default=0,
        help="Anzahl Worker-Prozesse. 0=auto (alle Kerne), 1=seriell",
    )
    ap.add_argument(
        "--lsq_max_nfev",
        type=int,
        default=20000,
        help="max_nfev for scipy.least_squares inside Molinaroli model",
    )
    ap.add_argument(
        "--ind_timeout_s",
        type=float,
        default=120.0,
        help="Timeout per population = ind_timeout_s * population. 0 disables.",
    )

    args = ap.parse_args()

    if not args.csv.exists():
        raise FileNotFoundError(args.csv)

    Path(args.out_dir).mkdir(parents=True, exist_ok=True)

    # --- parameter config ---
    param_names, default_params = get_param_config(args.model)

    # --- Daten laden ---
    df = read_dataset_csv(args.csv, args.sep, args.header, args.decimal)

    oil_sel = str(args.oil).strip().lower()
    if oil_sel != "all":
        if args.oil_col not in df.columns:
            raise ValueError(f"Öl-Spalte '{args.oil_col}' nicht gefunden.")
        df = df[df[args.oil_col].astype(str).str.strip().str.lower() == oil_sel].copy()

    required = [
        args.col_p_suc,
        args.col_T_suc,
        args.col_p_out,
        args.col_T_amb,
        args.col_speed,
        args.col_m_meas,
        args.col_P_meas,
    ]
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"Fehlende Spalten: {missing}")

    if args.model == "modified" and args.oil_col not in df.columns:
        raise ValueError("Modified model requires an oil column in the dataset.")

    df = df.dropna(subset=required).reset_index(drop=True)
    if df.empty:
        raise ValueError("Keine Daten nach NaN-Filter.")

    N_max_hz = rpm_to_hz(args.N_max_rpm)
    V_h_m3 = args.V_h_cm3 * 1e-6

    rows_all, has_Tdis = build_rows(df, args, N_max_hz)
    use_Tdis = bool(has_Tdis)
    if not use_Tdis:
        print(f"[INFO] Spalte '{args.col_T_dis}' nicht gefunden — T_dis nicht in Zielfunktion.")

    rng = np.random.default_rng(args.seed)
    rows_train, is_train_mask, n_train_units, n_total_units = select_training_rows(rows_all, args, rng)

    # --- RefProp (Hauptprozess, für m_dot_ref und finale Vorhersage) ---
    try:
        med = RefProp(fluid_name=args.refrigerant)
    except TypeError:
        med = RefProp(args.refrigerant)

    m_dot_ref = compute_m_dot_ref(med, V_h_m3)
    print(f"  m_dot_ref = {m_dot_ref * 1e3:.4f} g/s  (V_h={args.V_h_cm3} cm³, f_ref={F_REF} Hz)")

    # --- Bounds ---
    bounds = get_bounds(args.model, V_h_m3, args.vic_lo_scale, args.vic_hi_scale)

    # --- Initialpopulation ---
    x0 = np.clip(
        np.array([default_params[n] for n in param_names], dtype=float),
        bounds[:, 0],
        bounds[:, 1],
    )
    pop_size = int(args.population)
    elite_k = max(1, int(np.ceil(args.elite_frac * pop_size)))

    population: list[np.ndarray] = [x0.copy()]
    while len(population) < pop_size:
        population.append(random_individual(bounds, rng))

    # --- eval_pop: seriell oder parallel ---
    n_jobs = int(args.n_jobs)
    if n_jobs <= 0:
        n_jobs = os.cpu_count() or 1

    if n_jobs == 1:
        def eval_pop(pop):
            errs = np.empty(len(pop), dtype=float)
            for i, ind in enumerate(pop):
                errs[i] = objective_error(
                    x=ind,
                    rows=rows_train,
                    med=med,
                    model=args.model,
                    N_max_hz=N_max_hz,
                    V_h_m3=V_h_m3,
                    m_dot_ref=m_dot_ref,
                    use_Tdis=use_Tdis,
                    Tdis_norm_K=args.Tdis_norm_K,
                    lsq_max_nfev=args.lsq_max_nfev,
                    param_names=param_names,
                    default_params=default_params,
                    refrigerant_name=args.refrigerant,
                )
            return errs

        executor = None

    else:
        def make_executor():
            return ProcessPoolExecutor(
                max_workers=n_jobs,
                initializer=_init_worker,
                initargs=(
                    str(args.refrigerant),
                    str(args.model),
                    float(N_max_hz),
                    float(V_h_m3),
                    float(m_dot_ref),
                    rows_train,
                    bool(use_Tdis),
                    float(args.Tdis_norm_K),
                    int(args.lsq_max_nfev),
                    param_names,
                    default_params,
                ),
            )

        executor = make_executor()

        from concurrent.futures import wait

        def eval_pop(pop):
            nonlocal executor

            futs = {executor.submit(_objective_error_worker, ind): i for i, ind in enumerate(pop)}
            errs = np.full(len(pop), 3.0 * FAIL_E ** 2, dtype=float)

            timeout_s = float(args.ind_timeout_s)
            if timeout_s <= 0:
                for f, i in futs.items():
                    try:
                        errs[i] = float(f.result())
                    except Exception:
                        errs[i] = 3.0 * FAIL_E ** 2
                return errs

            total_budget = timeout_s * len(pop)
            done, not_done = wait(futs.keys(), timeout=total_budget)

            for f in done:
                i = futs[f]
                try:
                    errs[i] = float(f.result())
                except Exception:
                    errs[i] = 3.0 * FAIL_E ** 2

            if not_done:
                print(f"[WARN] Gesamt-Timeout: {len(not_done)} Individuen bestraft -> Pool Neustart")
                executor.shutdown(wait=False, cancel_futures=True)
                executor = make_executor()

            return errs

    print(f"  Parallelisierung: {'seriell' if n_jobs == 1 else f'{n_jobs} Worker-Prozesse'}")
    print(f"  Populationsgröße: {pop_size}  |  Eliten: {elite_k}  |  Generationen: {args.generations}")
    print(
        f"  Train split: mode={args.train_selection_mode}, "
        f"selected_units={n_train_units}/{n_total_units}, "
        f"train_rows={len(rows_train)}/{len(rows_all)}"
    )
    if args.shared_split_csv is not None:
        print(f"  Shared split CSV: {args.shared_split_csv}")

    # --- GA-Loop ---
    try:
        errors = eval_pop(population)
        best_x = population[int(np.argmin(errors))].copy()
        best_err = float(np.min(errors))
        print(f"[INIT] best_err={best_err:.6e}")

        best_snapshots: list[dict] = []

        for gen in range(1, args.generations + 1):
            order = np.argsort(errors)
            population = [population[i] for i in order]
            errors = errors[order]

            if float(errors[0]) < best_err:
                best_err = float(errors[0])
                best_x = population[0].copy()

            selected = list(population[:elite_k])
            selected_errs = list(errors[:elite_k].astype(float))

            for i in range(elite_k, pop_size):
                if rng.random() < args.random_keep_prob:
                    selected.append(population[i])
                    selected_errs.append(float(errors[i]))

            if len(selected) < 2:
                selected = population[:2]
                selected_errs = list(errors[:2].astype(float))

            rounded = np.round(np.asarray(selected_errs), decimals=12)
            seen, dup_idxs = {}, []
            for i, v in enumerate(rounded):
                if v in seen:
                    dup_idxs.append(i)
                else:
                    seen[v] = i
            if len(dup_idxs) >= 3:
                for i in dup_idxs:
                    if i >= 2:
                        selected[i] = random_individual(bounds, rng)

            children: list[np.ndarray] = []
            while len(children) < pop_size - elite_k:
                p1 = selected[int(rng.integers(0, len(selected)))]
                p2 = selected[int(rng.integers(0, len(selected)))]
                child = mutate(
                    uniform_crossover(p1, p2, rng),
                    bounds,
                    rng,
                    args.mutation_prob_param,
                )
                children.append(np.clip(child, bounds[:, 0], bounds[:, 1]))

            population = [population[i].copy() for i in range(elite_k)] + children
            errors = eval_pop(population)

            print(
                f"[GEN {gen:4d}] "
                f"best_gen={float(np.min(errors)):.6e}  "
                f"best_so_far={best_err:.6e}"
            )

            if gen % 10 == 0:
                snap = {"gen": int(gen), "best_err_so_far": float(best_err)}
                snap.update({name: float(val) for name, val in zip(param_names, best_x)})
                best_snapshots.append(snap)

                pretty = ", ".join([f"{k}={snap[k]:.6g}" for k in param_names])
                print(f"[BEST @ GEN {gen:4d}] err={best_err:.6e} | {pretty}")

    finally:
        if executor is not None:
            executor.shutdown(wait=True)

    # --- Vorhersagen exportieren ---
    tag = "ga"
    run_id = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    out_dir = Path(args.out_dir)

    params_best = x_to_params(best_x, param_names, default_params)
    params_best["f_ref"] = F_REF
    params_best["m_dot_ref"] = float(m_dot_ref)

    model_name = str(args.model).lower().strip()
    comp_pred_cache: dict[str, object] = {}

    def _get_pred_comp(row):
        if model_name in ("orig", "original"):
            key = "__single__"
            oil_name = None
        else:
            key = str(row["oil_norm"])
            oil_name = str(row["oil_norm"])

        if key not in comp_pred_cache:
            comp = make_compressor(
                model=args.model,
                N_max_hz=N_max_hz,
                V_h_m3=V_h_m3,
                params=params_best,
                refrigerant_name=args.refrigerant,
                oil_name=oil_name,
            )
            comp.med_prop = med
            comp_pred_cache[key] = comp
        return comp_pred_cache[key]

    inputs_pred = SimpleInputs(
        control=Control(n=1e-6),
        T_amb=298.15,
        lsq_max_nfev=20000,
        lsq_ftol=1e-8,
        lsq_xtol=1e-8,
    )
    fs_state_pred = FlowsheetState()

    pred_rows = []
    for i, r in enumerate(rows_all):
        ok = True
        try:
            comp_pred = _get_pred_comp(r)
            m_c, P_c, T_c = simulate_point(
                comp_pred,
                med,
                inputs_pred,
                fs_state_pred,
                r["p_suc_pa"],
                r["T_suc_K"],
                r["p_out_pa"],
                r["n_rel"],
                r["T_amb_K"],
            )
        except Exception:
            ok, m_c, P_c, T_c = False, np.nan, np.nan, np.nan

        pred_rows.append({
            "idx": i,
            "source_row_index": int(r["source_row_index"]),
            "is_train": bool(is_train_mask[i]),
            "oil": r["oil"],
            "oil_norm": r["oil_norm"],
            "op_key": r["op_key"],
            "f_oper_hz": r["f_oper_hz"],
            "p_suc_bar": r["p_suc_pa"] / 1e5,
            "T_suc_C": r["T_suc_K"] - 273.15,
            "p_out_bar": r["p_out_pa"] / 1e5,
            "T_amb_C": r["T_amb_K"] - 273.15,

            "m_meas_gps": r["m_meas"] * 1e3,
            "m_calc_gps": m_c * 1e3 if ok else np.nan,
            "e_m_rel": (m_c / r["m_meas"] - 1.0) if ok else np.nan,

            "P_meas_W": r["P_meas"],
            "P_calc_W": P_c if ok else np.nan,
            "e_P_rel": (P_c / r["P_meas"] - 1.0) if ok else np.nan,

            "T_dis_meas_C": (r["T_dis_meas_K"] - 273.15)
            if r.get("T_dis_meas_K") is not None else np.nan,
            "T_dis_calc_C": (T_c - 273.15) if ok else np.nan,
            "e_T_dis_K": (T_c - r["T_dis_meas_K"])
            if (ok and r.get("T_dis_meas_K") is not None) else np.nan,
            "ok": ok,
        })

    final_err = objective_error(
        x=best_x,
        rows=rows_train,
        med=med,
        model=args.model,
        N_max_hz=N_max_hz,
        V_h_m3=V_h_m3,
        m_dot_ref=m_dot_ref,
        use_Tdis=use_Tdis,
        Tdis_norm_K=args.Tdis_norm_K,
        lsq_max_nfev=args.lsq_max_nfev,
        param_names=param_names,
        default_params=default_params,
        refrigerant_name=args.refrigerant,
    )

    # --- Statistik ---
    df_pred = pd.DataFrame(pred_rows).dropna(subset=["e_m_rel", "e_P_rel"])
    m5 = (df_pred["e_m_rel"].abs() <= 0.05).mean() * 100 if not df_pred.empty else np.nan
    P5 = (df_pred["e_P_rel"].abs() <= 0.05).mean() * 100 if not df_pred.empty else np.nan
    print(f"\n  Punkte innerhalb ±5 % (Massenstrom): {m5:.1f} %")
    print(f"  Punkte innerhalb ±5 % (Leistung):    {P5:.1f} %")
    if use_Tdis and "e_T_dis_K" in df_pred.columns:
        T3 = (df_pred["e_T_dis_K"].abs() <= 3.0).mean() * 100
        print(f"  Punkte innerhalb ±3 K (T_dis):       {T3:.1f} %")

    # --- Speichern ---
    suffix = f"{str(args.oil).lower()}_{args.model}_{tag}_{run_id}"

    fitted_row = {k: float(v) for k, v in zip(param_names, best_x)}
    fitted_row.update({
        "f_ref": F_REF,
        "T_ref": T_REF,
        "m_dot_ref": float(m_dot_ref),
        "m_dot_ref_definition": "rho_sat(T=273.15K,Q=1)*V_h_geo*f_ref",
        "oil": str(args.oil),
        "refrigerant": str(args.refrigerant),
        "model": str(args.model),
        "error_sum_sq": float(final_err),
        "n_train": len(rows_train),                  # rows, kept for compatibility
        "n_points_total": len(rows_all),            # rows, kept for compatibility
        "n_train_units": int(n_train_units),        # rows or operating points
        "n_units_total": int(n_total_units),
        "train_selection_mode": str(args.train_selection_mode),
        "shared_split_csv": str(args.shared_split_csv) if args.shared_split_csv else "",
        "use_Tdis": bool(use_Tdis),
        "Tdis_norm_K": float(args.Tdis_norm_K),
        "seed": int(args.seed),
        "population": pop_size,
        "elite_frac": float(args.elite_frac),
        "random_keep_prob": float(args.random_keep_prob),
        "mutation_prob_param": float(args.mutation_prob_param),
        "generations": int(args.generations),
        "n_jobs": n_jobs,
        "log_uniform_params": "A_tot,A_dis",
        "fit_param_names": ",".join(param_names),
    })

    out_params = out_dir / f"fitted_params_{suffix}.csv"
    out_pred = out_dir / f"fit_predictions_{suffix}.csv"
    pd.DataFrame([fitted_row]).to_csv(out_params, index=False)
    pd.DataFrame(pred_rows).to_csv(out_pred, index=False)

    if best_snapshots:
        out_snap = out_dir / f"best_params_snapshots_{suffix}.csv"
        pd.DataFrame(best_snapshots).to_csv(out_snap, index=False)
        print(f" Best-snapshots gespeichert: {out_snap}")

    print("\n=== GA FIT DONE ===")
    print(f"  Finaler Fehler (Trainingsdaten): {final_err:.6e}")
    print(f"  Parameter gespeichert: {out_params}")
    print(f"  Vorhersagen gespeichert: {out_pred}")


if __name__ == "__main__":
    mp.freeze_support()
    main()