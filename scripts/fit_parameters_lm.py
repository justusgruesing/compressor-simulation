# Fits 8 Molinaroli parameters using SciPy least_squares (Levenberg–Marquardt).
# Input:  your CSV (with a units row -> header is 2nd line)
# Output: ONLY CSV files:
#   - results/fitted_params_<oil>_<model>[...].csv   (one row with parameter values)
#   - results/fit_predictions_<oil>_<model>[...].csv (measured vs predicted + relative residuals)
#
# Units (fixed to your dataset):
#   P1_mean, P2_mean: bar
#   T1_mean, Tamb_mean, T2_mean: °C
#   suction_mf_mean: g/s
#   Pel_mean: W
#   N: 1/min (rpm)
#
# Datasheet defaults:
#   N_max = 7200 rpm
#   V_h   = 30.7 cm^3
#
# NOTE about LM:
# - SciPy's Levenberg–Marquardt (method="lm") does NOT support bounds.
# - To keep bounds safely, this script uses an internal logistic transform:
#     z (unbounded)  ->  x (bounded)
#   LM optimizes z, while the model is evaluated with bounded x.

import argparse
from dataclasses import dataclass
from pathlib import Path

import numpy as np
import pandas as pd
from scipy.optimize import least_squares

from datetime import datetime

from vclibpy.media.cool_prop import CoolProp
from vclibpy.datamodels import FlowsheetState
from vclibpy.components.compressors import (
    Molinaroli_2017_Compressor,
    Molinaroli_2017_Compressor_Modified,
)

# =========================
# CSV columns
# =========================
OIL_COL = "Ölbezeichnung"

P_SUC_COL = "P1_mean"            # bar
T_SUC_COL = "T1_mean"            # °C
P_OUT_COL = "P2_mean"            # bar
T_AMB_COL = "Tamb_mean"          # °C
T_DIS_COL = "T2_mean"            # °C (measured discharge temperature)

M_FLOW_MEAS_COL = "suction_mf_mean"   # g/s
P_EL_MEAS_COL = "Pel_mean"            # W
SPEED_COL = "N"  # 1/min (rpm)

# =========================
# Fixed reference values
# =========================
F_REF = 50.0      # Hz
T_REF = 273.15    # K
Q_REF = 1.0       # saturated vapor

# =========================
# Inputs wrapper for model
# =========================
@dataclass
class Control:
    n: float  # relative speed 0..1

@dataclass
class SimpleInputs:
    control: Control
    T_amb: float  # K

# =========================
# 8 fit parameters
# =========================
PARAM_NAMES = [
    "Ua_suc_ref",
    "Ua_dis_ref",
    "Ua_amb",
    "A_tot",
    "A_dis",
    "V_IC",
    "alpha_loss",
    "W_dot_loss_ref",
]

DEFAULT_PARAMS = {
    "Ua_suc_ref": 16.05,
    "Ua_dis_ref": 13.96,
    "Ua_amb": 0.36,
    "A_tot": 9.47e-9,
    "A_dis": 86.1e-6,       # corrected
    "V_IC": 16.11e-6,
    "alpha_loss": 0.16,
    "W_dot_loss_ref": 83.0,
    "m_dot_ref": None,      # computed from paper definition
    "f_ref": F_REF,
}

# Bounds in the same order as PARAM_NAMES (used via internal transform)
LB = np.array([0.01, 0.01, 0.0,   1e-12, 1e-8, 1e-9,  0.0,  0.0], dtype=float)
UB = np.array([500.0, 500.0, 50.0, 1e-6,  1e-3,  1e-3,  1.0,  5000.0], dtype=float)


# =========================
# Bounded <-> unbounded transform for LM
# =========================
def _sigmoid(z: np.ndarray) -> np.ndarray:
    # numerically stable sigmoid
    z = np.asarray(z, dtype=float)
    out = np.empty_like(z)
    pos = z >= 0
    out[pos] = 1.0 / (1.0 + np.exp(-z[pos]))
    ez = np.exp(z[~pos])
    out[~pos] = ez / (1.0 + ez)
    return out

def z_to_x(z: np.ndarray, lb: np.ndarray, ub: np.ndarray) -> np.ndarray:
    """Map unbounded z -> bounded x in [lb, ub]."""
    s = _sigmoid(z)
    return lb + (ub - lb) * s

def x_to_z(x: np.ndarray, lb: np.ndarray, ub: np.ndarray) -> np.ndarray:
    """Map bounded x in [lb, ub] -> unbounded z. (with clipping for safety)"""
    x = np.asarray(x, dtype=float)
    span = (ub - lb)
    eps = 1e-12
    y = (x - lb) / span
    y = np.clip(y, eps, 1.0 - eps)
    return np.log(y / (1.0 - y))


def x_to_params(x: np.ndarray) -> dict:
    p = DEFAULT_PARAMS.copy()
    for name, val in zip(PARAM_NAMES, x):
        p[name] = float(val)
    return p


# =========================
# Unit conversion (fixed for your dataset)
# =========================
def bar_to_pa(p_bar: float) -> float:
    return float(p_bar) * 100000.0

def c_to_k(t_c: float) -> float:
    return float(t_c) + 273.15

def rpm_to_hz(rpm: float) -> float:
    return float(rpm) / 60.0

def gs_to_kgps(g_s: float) -> float:
    return float(g_s) / 1000.0


# =========================
# Model selection
# =========================
def make_compressor(model: str, N_max_hz: float, V_h_m3: float, params: dict):
    m = model.lower().strip()
    if m in ("orig", "original"):
        return Molinaroli_2017_Compressor(N_max=N_max_hz, V_h=V_h_m3, parameters=params)
    if m in ("mod", "modified"):
        return Molinaroli_2017_Compressor_Modified(N_max=N_max_hz, V_h=V_h_m3, parameters=params)
    raise ValueError("Unknown --model. Use: original | modified")


# =========================
# calculation of m_dot_ref
# =========================
def compute_m_dot_ref(med: CoolProp, V_h_m3: float) -> float:
    st = med.calc_state("TQ", T_REF, Q_REF)
    rho_ref = float(st.d)
    return rho_ref * float(V_h_m3) * float(F_REF)


def simulate_point(
    med: CoolProp, model: str, params_base: dict,
    N_max_hz: float, V_h_m3: float,
    p_suc_pa: float, T_suc_K: float, p_out_pa: float,
    f_oper_hz: float, T_amb_K: float
):
    # relative speed expected by VCLibPy baseclass
    n_rel = f_oper_hz / N_max_hz
    n_rel = max(1e-6, min(1.0, n_rel))

    params = dict(params_base)
    params["f_ref"] = F_REF
    params["m_dot_ref"] = compute_m_dot_ref(med, V_h_m3)

    comp = make_compressor(model=model, N_max_hz=N_max_hz, V_h_m3=V_h_m3, params=params)
    comp.med_prop = med
    comp.state_inlet = med.calc_state("PT", p_suc_pa, T_suc_K)

    inputs = SimpleInputs(control=Control(n=n_rel), T_amb=T_amb_K)
    fs_state = FlowsheetState()
    comp.calc_state_outlet(p_outlet=p_out_pa, inputs=inputs, fs_state=fs_state)

    T_dis_K = float(comp.state_outlet.T)
    return float(comp.m_flow), float(comp.P_el), T_dis_K


# Residuals (relative errors, equal weighting)
# Default: m_flow + P_el
# Optional: + T_dis (if use_t_dis=True)
def residuals_for_dataset(
    x: np.ndarray, rows: list[dict], med: CoolProp,
    model: str, N_max_hz: float, V_h_m3: float,
    use_t_dis: bool
) -> np.ndarray:
    params_base = x_to_params(x)
    res = []

    for row in rows:
        try:
            m_calc, P_calc, T_dis_calc = simulate_point(
                med=med, model=model, params_base=params_base,
                N_max_hz=N_max_hz, V_h_m3=V_h_m3,
                p_suc_pa=row["p_suc_pa"], T_suc_K=row["T_suc_K"], p_out_pa=row["p_out_pa"],
                f_oper_hz=row["f_oper_hz"], T_amb_K=row["T_amb_K"],
            )

            m_meas = row["m_meas"]
            P_meas = row["P_meas"]

            res.append((m_calc / m_meas) - 1.0)
            res.append((P_calc / P_meas) - 1.0)

            if use_t_dis:
                T_dis_meas = row["T_dis_meas_K"]
                res.append((T_dis_calc / T_dis_meas) - 1.0)

        except Exception:
            res.extend([10.0, 10.0] + ([10.0] if use_t_dis else []))

    return np.asarray(res, dtype=float)


def read_dataset_csv(path: Path) -> pd.DataFrame:
    return pd.read_csv(path, sep=";", header=1, decimal=",")


def load_x0_csv(path: Path) -> np.ndarray:
    """
    Load start values for the 8 fit parameters from start_params.csv (one row).
    """
    df0 = pd.read_csv(path)
    if len(df0) != 1:
        raise ValueError("x0 CSV must contain exactly one row.")

    missing = [n for n in PARAM_NAMES if n not in df0.columns]
    if missing:
        raise ValueError(f"x0 CSV missing columns: {missing}")

    row = df0.iloc[0]
    return np.array([float(row[n]) for n in PARAM_NAMES], dtype=float)


def main():
    ap = argparse.ArgumentParser(
        description="Fit Molinaroli parameters (SciPy least_squares, Levenberg–Marquardt). Residuals: m_flow, P_el (default) + optional T_dis."
    )

    ap.add_argument("--csv", required=True, help="Path to dataset CSV")
    ap.add_argument("--model", default="original", help="original | modified")
    ap.add_argument("--refrigerant", default="R290")

    ap.add_argument("--x0_csv", default=None, help="One-row CSV with start values for the 8 parameters")
    ap.add_argument("--oil", default="all", help="LPG100 | LPG68 | all")

    # Default OFF now:
    ap.add_argument(
        "--use_t_dis",
        type=int,
        default=0,
        choices=[0, 1],
        help="0 = fit only m_flow and P_el (default). 1 = include T_dis as additional residual (needs T2_mean)."
    )

    # Datasheet defaults:
    ap.add_argument("--N_max_rpm", type=float, default=7200.0, help="Max speed [1/min] (default: 7200)")
    ap.add_argument("--V_h_cm3", type=float, default=30.7, help="Displacement volume [cm^3] (default: 30.7)")

    ap.add_argument("--max_rows", type=int, default=None, help="Optional limit for quick tests")
    ap.add_argument("--max_nfev", type=int, default=400, help="Max function evals")

    # Outputs (CSV only)
    ap.add_argument("--out_params_csv", default=None, help="Output CSV for fitted params (one row)")
    ap.add_argument("--out_pred_csv", default=None, help="Output CSV with predictions/residuals")

    # Debugging and tolerances
    ap.add_argument("--ftol", type=float, default=1e-8, help="Function tolerance")
    ap.add_argument("--xtol", type=float, default=1e-10, help="Parameter tolerance")
    ap.add_argument("--gtol", type=float, default=1e-8, help="Gradient tolerance")
    ap.add_argument("--debug", action="store_true", help="Print debug info every function evaluation")

    args = ap.parse_args()
    use_t_dis = bool(args.use_t_dis)

    csv_path = Path(args.csv)
    if not csv_path.exists():
        raise FileNotFoundError(csv_path)

    df = read_dataset_csv(csv_path)

    oil_sel = args.oil.strip().lower()
    if oil_sel != "all":
        df = df[df[OIL_COL].astype(str).str.lower() == oil_sel]

    if args.max_rows is not None:
        df = df.head(args.max_rows)

    # Required columns depend on use_t_dis
    required = [OIL_COL, P_SUC_COL, T_SUC_COL, P_OUT_COL, T_AMB_COL,
                M_FLOW_MEAS_COL, P_EL_MEAS_COL, SPEED_COL]
    if use_t_dis:
        required.append(T_DIS_COL)

    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"CSV missing columns: {missing}")

    df = df.dropna(subset=required).reset_index(drop=True)
    if len(df) == 0:
        raise ValueError("No rows left after filtering / NaN removal.")

    # Convert datasheet values
    N_max_hz = rpm_to_hz(args.N_max_rpm)
    V_h_m3 = float(args.V_h_cm3) * 1e-6  # cm^3 -> m^3

    # Prepare rows for fitting
    rows = []
    for _, r in df.iterrows():
        p_suc_pa = bar_to_pa(r[P_SUC_COL])
        p_out_pa = bar_to_pa(r[P_OUT_COL])
        T_suc_K = c_to_k(r[T_SUC_COL])
        T_amb_K = c_to_k(r[T_AMB_COL])

        f_oper_hz = rpm_to_hz(r[SPEED_COL])

        m_meas = gs_to_kgps(r[M_FLOW_MEAS_COL])
        P_meas = float(r[P_EL_MEAS_COL])

        if m_meas <= 0 or P_meas <= 0:
            continue

        rowd = {
            "oil": r[OIL_COL],
            "p_suc_pa": p_suc_pa,
            "T_suc_K": T_suc_K,
            "p_out_pa": p_out_pa,
            "T_amb_K": T_amb_K,
            "f_oper_hz": f_oper_hz,
            "m_meas": m_meas,
            "P_meas": P_meas,
        }

        if use_t_dis:
            T_dis_meas_K = c_to_k(r[T_DIS_COL])
            if T_dis_meas_K <= 0:
                continue
            rowd["T_dis_meas_K"] = T_dis_meas_K

        rows.append(rowd)

    if len(rows) == 0:
        raise ValueError("No valid rows after unit conversion/filtering.")

    # LM requirement: number of residuals >= number of variables
    n_res = len(rows) * (2 + (1 if use_t_dis else 0))
    if n_res < len(PARAM_NAMES):
        raise ValueError(
            f"Levenberg–Marquardt requires #residuals >= #variables. "
            f"Got residuals={n_res}, variables={len(PARAM_NAMES)}. "
            f"Use more data points or switch back to TRF."
        )

    med = CoolProp(fluid_name=args.refrigerant)

    # Default start values (bounded x)
    x0 = np.array([DEFAULT_PARAMS[n] for n in PARAM_NAMES], dtype=float)

    # Optional: start values from CSV (bounded x)
    if args.x0_csv is not None:
        x0_path = Path(args.x0_csv)
        if not x0_path.exists():
            raise FileNotFoundError(x0_path)
        x0 = load_x0_csv(x0_path)

    # Clip x0 into bounds (for the transform)
    x0 = np.maximum(LB, np.minimum(UB, x0))

    # Convert to unbounded z0 for LM
    z0 = x_to_z(x0, LB, UB)

    last_z = {"z": None}
    call_counter = {"n": 0}

    def fun(z):
        # map LM variable z -> bounded parameters x
        x = z_to_x(z, LB, UB)

        r = residuals_for_dataset(
            x=x, rows=rows, med=med,
            model=args.model, N_max_hz=N_max_hz, V_h_m3=V_h_m3,
            use_t_dis=use_t_dis
        )

        call_counter["n"] += 1

        if args.debug:
            cost = 0.5 * float(np.dot(r, r))
            dz = np.zeros_like(z) if last_z["z"] is None else (z - last_z["z"])

            print("\n--- fun(z) call", call_counter["n"], "---")
            print(f"cost = {cost:.6e}")
            print("x (bounded) and Δz (LM step):")
            for name, xi, dzi in zip(PARAM_NAMES, x, dz):
                print(f"  {name:>14s} = {xi: .6e}   Δz={dzi: .3e}")

            last_z["z"] = z.copy()

        return r

    print("Using x0 (bounded):", dict(zip(PARAM_NAMES, x0)))
    print("use_t_dis:", use_t_dis)

    # Levenberg–Marquardt (MINPACK)
    result = least_squares(
        fun,
        x0=z0,
        method="lm",
        x_scale="jac",   # helps with scaling for LM; optional
        ftol=args.ftol,
        xtol=args.xtol,
        gtol=args.gtol,
        max_nfev=args.max_nfev,
        verbose=2,
    )

    # Convert result back to bounded parameter vector x_hat
    x_hat = z_to_x(result.x, LB, UB)

    # Build fitted params dict
    fitted = x_to_params(x_hat)
    fitted["f_ref"] = F_REF
    fitted["T_ref"] = T_REF
    fitted["m_dot_ref_definition"] = "rho_sat_vapor(T=273.15K,Q=1)*V_h*f_ref"
    fitted["oil_fit_mode"] = args.oil
    fitted["N_max_rpm"] = args.N_max_rpm
    fitted["N_max_hz"] = N_max_hz
    fitted["V_h_cm3"] = args.V_h_cm3
    fitted["V_h_m3"] = V_h_m3
    fitted["refrigerant"] = args.refrigerant
    fitted["model"] = args.model
    fitted["use_t_dis"] = int(use_t_dis)
    fitted["success"] = bool(result.success)
    fitted["message"] = str(result.message)
    fitted["cost"] = float(result.cost)
    fitted["n_points"] = int(len(rows))

    # Output paths
    run_id = datetime.now().strftime("%Y-%m-%d")
    algo_tag = "lm"

    Path("results").mkdir(parents=True, exist_ok=True)
    suffix = "_with_t_dis" if use_t_dis else ""
    if args.out_params_csv is None:
        args.out_params_csv = f"results/fitted_params_{args.oil.lower()}_{args.model.lower()}{algo_tag}{suffix}_{run_id}.csv"
    if args.out_pred_csv is None:
        args.out_pred_csv = f"results/fit_predictions_{args.oil.lower()}_{args.model.lower()}{algo_tag}{suffix}_{run_id}.csv"

    pd.DataFrame([fitted]).to_csv(args.out_params_csv, index=False)

    # Save predictions
    pred = []
    for row in rows:
        m_calc, P_calc, T_dis_calc = simulate_point(
            med=med, model=args.model, params_base=fitted,
            N_max_hz=N_max_hz, V_h_m3=V_h_m3,
            p_suc_pa=row["p_suc_pa"], T_suc_K=row["T_suc_K"], p_out_pa=row["p_out_pa"],
            f_oper_hz=row["f_oper_hz"], T_amb_K=row["T_amb_K"],
        )

        out = {
            "oil": row["oil"],
            "f_oper_hz": row["f_oper_hz"],
            "p_suc_bar": row["p_suc_pa"] / 1e5,
            "T_suc_C": row["T_suc_K"] - 273.15,
            "p_out_bar": row["p_out_pa"] / 1e5,
            "T_amb_C": row["T_amb_K"] - 273.15,

            "m_meas_gps": row["m_meas"] * 1000.0,
            "m_calc_gps": m_calc * 1000.0,
            "e_m_rel": (m_calc / row["m_meas"]) - 1.0,

            "P_meas_W": row["P_meas"],
            "P_calc_W": P_calc,
            "e_P_rel": (P_calc / row["P_meas"]) - 1.0,
        }

        if use_t_dis:
            out.update({
                "T_dis_meas_C": row["T_dis_meas_K"] - 273.15,
                "T_dis_calc_C": T_dis_calc - 273.15,
                "e_T_dis_rel": (T_dis_calc / row["T_dis_meas_K"]) - 1.0,
            })

        pred.append(out)

    pd.DataFrame(pred).to_csv(args.out_pred_csv, index=False)

    print("\n=== FIT DONE (LM) ===")
    print("oil:", args.oil)
    print("success:", result.success)
    print("message:", result.message)
    print("cost:", result.cost)
    print("use_t_dis:", use_t_dis)
    print("saved params CSV:", args.out_params_csv)
    print("saved predictions CSV:", args.out_pred_csv)


if __name__ == "__main__":
    main()
