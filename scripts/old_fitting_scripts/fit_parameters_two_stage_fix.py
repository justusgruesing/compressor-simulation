# scripts/fit_parameters_two_stage_fix_A.py
#
# Fits Molinaroli parameters using SciPy least_squares (single or staged).
# Two-stage variant:
#   - Stage 1: A_tot, A_dis, V_IC are FIXED (not fitted). Fit the remaining 5 parameters.
#   - Stage 2: Fit ALL 8 parameters.
#
# Input:  CSV (with a units row -> header is 2nd line)
# Output: ONLY CSV files:
#   - results/fitted_params_<oil>_<model>_<tag>_<date>.csv
#   - results/fit_predictions_<oil>_<model>_<tag>_<date>.csv
#
# Residuals always include:
#   - m_dot (relative)
#   - Pel  (relative)
#   - T_dis (relative, weighted by w_T_dis)
#
...
# Bounds (same order as PARAM_NAMES)
...
# Manual scaling (same order as PARAM_NAMES)
X_SCALE_MANUAL_ALL = np.array([
    16.0,   # Ua_suc_ref
    14.0,   # Ua_dis_ref
    0.36,   # Ua_amb
    1e-8,   # A_tot
    1e-4,   # A_dis ~ O(1e-4..1e-5)
    1e-5,   # V_IC
    0.16,   # alpha_loss
    83.0,   # W_dot_loss_ref
], dtype=float)
...
# Two-stage setup
# =========================
# Stage 1: FIX A_tot, A_dis, V_IC  -> fit remaining 5 params
STAGE_1 = ["Ua_suc_ref", "Ua_dis_ref", "Ua_amb", "alpha_loss", "W_dot_loss_ref"]
# Stage 2: fit all
STAGE_2 = PARAM_NAMES[:]

# =========================
# Helpers for slicing/packing
# =========================
...
# =========================
# Residuals (ALWAYS: m_dot, Pel, T_dis)
# =========================
def residuals_for_dataset(
    ...
    for row in rows:
        try:
            ...
            res.append(w_t_dis * ((T_dis_calc / T_dis_meas) - 1.0))
...
def run_least_squares_for_names(
    ...
    def fun(x):
        ...
        if debug:
            ...
            print(f"\n--- {stage_name}: fun(x) call {call_counter['n']} ---")
    ...
    print(f"  x_scale   : {x_scale_mode}")
    print(f"  residuals : m_dot + Pel + T_dis (w_T_dis={w_t_dis})")
    ...
    print(f"\n=== {stage_name} RESULT (updated params) ===")
    for n in fit_names:
        print(f"{n:>14s} = {params_fitted_full[n]:.6e}")
...
def main():
    ap = argparse.ArgumentParser(
        description="Fit Molinaroli parameters (SciPy least_squares), TWO-STAGE: Stage1 fixes A_tot/A_dis/V_IC, Stage2 fits all."
    )
    ...
    ap.add_argument("--staged", action="store_true", help="Run TWO-STAGE fit: Stage1 (A fixed) -> Stage2 (all)")
    ap.add_argument("--x_scale", default="manual", choices=["manual", "jac"], help="Parameter scaling (manual array or jac)")

    ap.add_argument("--w_T_dis", type=float, default=1.0, help="Weight factor for T_dis residual")
    ...
    if args.staged:
        # Stage 1: A_tot/A_dis/V_IC are fixed because they are NOT in fit_names
        p1, _ = run_least_squares_for_names(
            stage_name="STAGE 1 (A_tot/A_dis/V_IC FIXED)",
            fit_names=STAGE_1,
            params_start_full=params_start,
            rows=rows, med=med, model=args.model,
            N_max_hz=N_max_hz, V_h_m3=V_h_m3,
            w_t_dis=args.w_T_dis,
        ...
        params_fitted, result = run_least_squares_for_names(
            stage_name="STAGE 2 (ALL PARAMETERS)",
            fit_names=STAGE_2,
        ...
        # Single fit over all
        params_fitted, result = run_least_squares_for_names(
            stage_name="SINGLE (ALL PARAMETERS)",
    ...
    print("oil:", args.oil)
    print("fit_mode:", tag)
    print("residuals: m_dot + Pel + T_dis (w_T_dis=", args.w_T_dis, ")")
